...
def create_grid():
    ...
    while num_of_ships_placed != num_of_ships :
        ...
        if try_to_place_ship_on_grid(random_row, random_col, random_direction, random_length):
...
def accept_valid_bullet_placement():
    ...
    while is_valid == False :    
        ...
        if len(placement) <= 1 or (len(placement) > 2) :
            print ("Error: Pleakse enter only one row and column such as A3")
            continue
...
def shoot_bullet():
    ...
    if grid[row][col] == "." :
        print ("you missed, no ship was there!")
        grid[row][col] = "#"    
    elif grid[row][col] == "O" :
        print ("You Hit! ", end ="")
        grid[row][col] = "X"
